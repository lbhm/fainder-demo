...
def generate_indices(base_path: Path) -> None:
    ...
    hists : list[tuple[np.uint32, Histogram]] = list()
                ...
                for recordSet in metadata["recordSet"]:
                    for column in recordSet["field"]:
                        if "histogram" in column:
                            ...
                            # convert bins to NDArray[np.float32]
                            bins = np.array(bins, dtype=np.float32)

                            # convert densities to NDArray[np.float64]
                            densities = np.array(densities, dtype=np.float64)

                            hists.append((np.uint32(record_number), (bins, densities)))
            except KeyError:
    ...
    clustered_hists, cluster_bins, features = cluster_histograms(hists, transform, quantile_range, True, algorithm, n_cluster_range, bin_budget,alpha, seed, os.cpu_count(), True )


    # create the rebinning index
    rebinning: Literal['rebinning', 'rebinning-shm', 'conversion', 'conversion-shm'] = "rebinning"
    conversion: Literal['rebinning', 'rebinning-shm', 'conversion', 'conversion-shm'] = "conversion"
    ...
    try:
        pctl_index_rebinning, _, shm_pointers = create_index(clustered_hists, cluster_bins, rebinning, index_precision, bin_estimation, os.cpu_count())

        pctl_index_conversion, _, shm_pointers2 = create_index(clustered_hists, cluster_bins, conversion, index_precision, bin_estimation, os.cpu_count())
    ...
    save_output(base_path / "fainder" / "conversion.zst", (pctl_index_rebinning, cluster_bins), name="index")

    save_output(base_path / "fainder" / "rebinning.zst", (pctl_index_conversion, cluster_bins), name="index")

def parse_args():
    parser = argparse.ArgumentParser(description="Generate indices for the histograms")
    parser.add_argument("-p", "--path", type=Path, help="Path to the directory containing the croissant directory")
